/*-------------------------------------------------------------------------
  ...
  LED devices such as Adafruit NeoPixel s.
  Currently handles 800 KHz and 400kHz bitstream on Spark Core and Photon,
  WS2812, WS2812B and WS2811.

  Also supports:
  - Radio Shack Tri-Color circle with TM1803 controller 400kHz bitstream.
  - TM1829 pixels
...
void Adafruit_NeoPixel::show(void) {
  ...
  else if(type == TM1803) { // TM1803 (Radio Shack Tri-Color circle), 400 KHz bitstream
    while(i) { // While bytes left... (3 bytes = 1 pixel)
      mask = 0x800000; // reset the mask
      i = i-3;      // decrement bytes remaining
      r = *ptr++;   // Next red byte value
...
// change, you'll need to re-render the full circle data.  C'est la vie.
